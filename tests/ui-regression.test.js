...
test("observation height measurement is accessible from More and requires explicit confirmation", () => {
  ...
  assert.ok(html.includes('id="observation-height-camera-basis" role="status" aria-live="polite" hidden'));
  ...
  assert.ok(measurementController.includes("現在のカメラ高（地面から）を基準に計算します。現在"));
  assert.ok(measurementController.includes("cameraHeight.toFixed(1)"));
