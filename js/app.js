import { createStore } from "./state.js?v=1.9.2";
import { createMapController, focusCurrentLocation } from "./map/map-controller.js?v=1.9.2";
import { bindPlaceSearch } from "./map/place-search.js?v=1.9.2";
import { loadRuntimeConfig } from "./config/runtime-config.js?v=1.9.2";
import { bindDateTimeControls } from "./ui/datetime-controls.js?v=1.9.2";
import { bindSkyStateRail } from "./ui/sky-state-rail.js?v=1.9.2";
import { normalizeThemePreference, resolveThemePreference, themeColor } from "./ui/theme.js?v=1.9.2";
import { calculateSunData } from "./astronomy/sun-service.js?v=1.9.2";
import { calculateMoonData } from "./astronomy/moon-service.js?v=1.9.2";
import { calculateMilkyWay } from "./astronomy/milky-way-service.js?v=1.9.2";
import { calculateSelectedTargets } from "./astronomy/target-service.js?v=1.9.2";
import { targetLabelList } from "./astronomy/target-catalog.js?v=1.9.2";
import { subjectGeometry } from "./geometry/bearing.js?v=1.9.2";
import { signedAngleDifference } from "./geometry/angle.js?v=1.9.2";
import { formatDistance } from "./utils/format.js?v=1.9.2";
import { bindSearchControls } from "./search/search-controller.js?v=1.9.2";
import { bindPlanManager } from "./plans/plan-manager.js?v=1.9.2";
import { createPlanRepository } from "./plans/plan-repository.js?v=1.9.2";
import { createPlanSyncCoordinator } from "./cloud/plan-sync.js?v=1.9.2";
import { bindCloudAccount } from "./cloud/account-controller.js?v=1.9.2";
import { parseSharedState } from "./plans/plan-data.js?v=1.9.2";
import { calculateComposition, focalLengthForFill, SENSOR_PRESETS } from "./composition/composition.js?v=1.9.2";
import { bindCompositionControls } from "./ui/composition-controls.js?v=1.9.2";
import { bindElevationControls } from "./elevation/elevation-controller.js?v=1.9.2";
import { apparentSolarAltitude, calculateTargetAltitude } from "./geometry/target-altitude.js?v=1.9.2";
import { bindShootingPlanner } from "./planning/shooting-planner.js?v=1.9.2";
import { bindTerrainProfile } from "./terrain/terrain-profile-controller.js?v=1.9.2";
import { bindFieldMode } from "./field/field-mode.js?v=1.9.2";
import { bindObservationHeightMeasurement } from "./measurement/observation-height-controller.js?v=1.9.2";
import { bindWeatherOverlay } from "./weather/weather-controller.js?v=1.9.2";
import { bindTargetSelector, moveTargetToPrimary } from "./ui/target-selector.js?v=1.9.2";
import { bindLightPollutionOverlay } from "./light-pollution/light-pollution-controller.js?v=1.9.2";
import { bindPwaRuntime } from "./pwa/pwa-runtime.js?v=1.9.2";
import { createOfflinePreparation } from "./plans/offline-preparation.js?v=1.9.2";
import { createFieldDataCollector, stateWithOfflineElevation } from "./field/field-data-collector.js?v=1.9.2";
import { restoreOfflineLight } from "./light-pollution/offline-light-pollution.js?v=1.9.2";
...
function setActiveDeckTab(panelId, { focus = false, persist = true } = {}) {
  ...
  const quickTimeControls = document.querySelector("#quick-time-controls");
  if (quickTimeControls) quickTimeControls.hidden = activeDeckTabId === "time-panel";
...
document.querySelector("#quick-time-date")?.addEventListener("click", () => {
  setActiveDeckTab("time-panel");
  renderState(store.getState());
...
function renderStellarTargets(targetData) {
  ...
  targetData.filter((data) => !["sun", "moon", "milkyway"].includes(data.target.id)).forEach((data) => {
    ...
    if (existing) {
      ...
      const metrics = existing.querySelectorAll(".metric-pair b");
      metrics[0].textContent = data.azimuth.toFixed(1);
      metrics[1].textContent = data.altitude.toFixed(1);
      existing.querySelector(".metric-pair em").textContent = data.direction;
      return;
    }
    ...
    heading.title = data.target.label;
...
function renderAlignment(state, targetData = []) {
  ...
  metrics.replaceChildren(...candidates.map((candidate) => {
    ...
    label.className = "alignment-target-label";
    label.style.setProperty("--target-color", candidate.color);
...
function renderState(state) {
  ...
  document.querySelector("#alignment-card").dataset.available = String(Boolean(state.subjectLocation && state.selectedTargets.length));
  document.querySelector("#composition-card").dataset.available = String(Boolean(state.subjectLocation));
  syncDeckTabs(state);
  const renderKey = JSON.stringify([state.selectedDateTime, state.cameraLocation, state.subjectLocation,
    state.selectedTargets, state.subject, state.composition, directionLineOrigin, Boolean(mapController), activeDeckTabId]);
  if (renderKey === lastRenderKey) return;
  lastRenderKey = renderKey;
  const targetKey = JSON.stringify([state.selectedDateTime, state.cameraLocation, state.selectedTargets]);
  if (targetKey !== lastTargetKey) {
    try {
      lastTargetData = calculateSelectedTargets(state.selectedTargets, new Date(state.selectedDateTime), state.cameraLocation);
  ...
  const targetData = lastTargetData;
  if (activeDeckTabId === "celestial-panel" && lastCelestialKey !== targetKey) {
    ...
    lastCelestialKey = targetKey;
  }
  const directionKey = JSON.stringify([targetKey, state.subjectLocation, directionLineOrigin, Boolean(mapController)]);
  if (directionKey !== lastDirectionKey) {
    renderCelestialDirections(state, targetData);
    if (state.subjectLocation) mapController?.setSubjectLocation(state.cameraLocation, state.subjectLocation);
    else mapController?.clearSubjectLocation();
    lastDirectionKey = directionKey;
  }
  if (activeDeckTabId === "alignment-card") renderAlignment(state, targetData);
  if (activeDeckTabId === "composition-card") renderComposition(state, targetData);
...
import { safeStorage as localStorage } from "./utils/storage.js?v=1.9.2";
